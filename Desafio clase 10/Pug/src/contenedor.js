...
class Contenedor {
    ...
    async modifyById(product, id) {
        const contenidoArchivo = await this.#leerArchivo()
        try {
          const { title, price, thumbnail } = product
          const item = contenidoArchivo.find((prod) => prod.id === Number(id))
          if (item) {
            item.title = title
            item.price = price
    ...
    //     const contenidoArchivo = await this.#leerArchivo()
    //     try {
    //         const productoPut = await contenidoArchivo.map(prod => {
    //             if (prod.id === id) {
    //                 prod = {
    //                     ...product,
    //                     id: id
    ...
    //             }
    //         })
    //         await fs.promises.writeFile(this.file, JSON.stringify(contenidoArchivo, null, 2), 'utf-8');
